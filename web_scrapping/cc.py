...
for i in range(len(product_names)):
    ...
    if len(net_weight) == len(product_name):
        df=pd.DataFrame(data=list(zip(gross_weight,product_code,net_weight,totalPrice)),columns=['Gross Weight','Product Code','Net Weight','Selling Price'])
        df['Category'] = product_names[i]
        df['Competitor Name'] = comp_name
        df['Unit of Measurement'] = 'gm'
        df=df[['Competitor Name','Category','Product Code','Gross Weight','Net Weight','Selling Price','Unit of Measurement']]
    else:
        df=pd.DataFrame(data=list(zip(weights,product_code,totalPrice)),columns=['Weight','Product Code','Selling Price'])
        df['Category'] = product_names[i]
        df['Competitor Name']=comp_name
        df['Unit of Measurement'] = 'gm'
        df=df[['Competitor Name','Category','Product Code','Weight','Selling Price','Unit of Measurement']]
    path=f'/mnt/c/python_projects/fipola/cc.csv'
